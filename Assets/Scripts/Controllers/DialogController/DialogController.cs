...
public class DialogController : MonoBehaviour
{
    ...
    /// 获取用于测试的对话数据
    /// </summary>
    private DialogueData GetDialogueDataForTest()
    {
        // 优先使用自定义对话数据
        if (customDialogueData != null)
        {
            LogDebug($"使用自定义对话数据: {customDialogueData.dialogueId}");
            return customDialogueData;
        }
        
        // 如果没有自定义数据，使用默认测试数据
        LogDebug("使用默认测试对话数据");
        return CreateTestDialogueData();
    }

    /// <summary>
    /// 创建测试对话数据
    /// </summary>
    private DialogueData CreateTestDialogueData()
    {
        DialogueData dialogueData = ScriptableObject.CreateInstance<DialogueData>();
        dialogueData.dialogueId = "TestDialogue";
        ...
        DialogueEntry[] entries = new DialogueEntry[]
        {
            ...
            {
                speakerName = "测试角色",
                dialogueText = "这是一个测试对话，用于验证对话系统是否正常工作。",
                portrait = null,
            ...
            {
                speakerName = "系统",
                dialogueText = "如果这个对话正常显示，说明对话系统配置正确。",
                portrait = null,
                portraitPosition = UI_DialogView.PortraitPosition.Right
            }
        };
        
        dialogueData.dialogueEntries = entries;
        LogDebug($"创建测试对话数据，包含 {entries.Length} 个条目");
        return dialogueData;
    }
